...
// IOStream device that operates without SSL
template <typename Protocol>
class IOStreamDevice : public iostreams::device<iostreams::bidirectional> {
public:
    IOStreamDevice(typename Protocol::socket &socketIn) : socket(socketIn) {}
    //IOStreamDevice(asio::io_context& io_context) : socket(io_context) {}

    std::streamsize read(char* s, std::streamsize n) {
        return socket.read_some(asio::buffer(s, n));
    }

    std::streamsize write(const char* s, std::streamsize n) {
        return asio::write(socket, asio::buffer(s, n));
    }
...
bool connect(const std::string& server, const std::string& port) {
    ...
    boost::asio::ip::address address = boost::asio::ip::address::from_string(server, error);
    if (!error) {
        // If 'server' is an IP address, create an endpoint directly
        endpoint = boost::asio::ip::tcp::endpoint(address, std::stoi(port));
    } else {
        // If 'server' is a hostname, resolve it to get the endpoint
        return false;
    }
...
public:
    AcceptedConnectionImpl(asio::io_context& io_context)
        : socket(io_context), device(socket), io_stream(device) {
...
private:
    IOStreamDevice<Protocol> device;
    iostreams::stream<IOStreamDevice<Protocol>> io_stream;  // Renamed from 'stream' to 'io_stream'
};
...
static void RPCListen(boost::shared_ptr<basic_socket_acceptor<Protocol, SocketAcceptorService>> acceptor)
{
    ...
    asio::io_context& io_context = static_cast<asio::io_context&>(acceptor->get_executor().context());
    AcceptedConnectionImpl<Protocol>* conn = new AcceptedConnectionImpl<Protocol>(io_context);
...
void ThreadRPCServer2(void* parg)
{
    ...
    try
    {
        boost::shared_ptr<ip::tcp::acceptor> acceptor(new ip::tcp::acceptor(io_service, endpoint));
        acceptor->listen();
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    IOStreamDevice<asio::ip::tcp> d(socket);
    iostreams::stream<IOStreamDevice<asio::ip::tcp>> stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "19918")))
        throw runtime_error("couldn't connect to server");
